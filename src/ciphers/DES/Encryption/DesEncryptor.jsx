...
const DesSimulator = () => {
  ...
  const handleConvert = (e) => {
    e.preventDefault();
    try {
      const result = DesEncryption(plainText, keyText);
      setState(result);
      setPlainText(result.paddedInput);
  ...
  return (
    <div className="des-container">
        ...
        <div className="form-grid">
          <div className="form-left">
            <div>
              ...
              <input type="text" value={plainText} onChange={(e) => setPlainText(e.target.value)} maxLength={8} placeholder="8 ASCII chars" />
            </div>
            <div>
              <label>Key</label>
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 4 && <motion.div key="step4" initial={{ opacity: 0 }} animate={{ opacity: 1 }}><h3>Step 4: Round 1</h3><HintBox step={4} /><Step4Round1 L0Bits={state.L0} R0Bits={state.R0} K1Bits={state.K1} expandedR={state.expandedR0} xorWithKey={state.xorR0K1} sboxOutput={state.sboxR0} pboxOutput={state.pboxR0} R1Bits={state.R1} /><button onClick={() => { unlockNextStep(); setStep(5); }} style={{ marginTop: '1rem' }}>Next Step</button></motion.div>}
        {step === 5 && <motion.div key="step5" initial={{ opacity: 0 }} animate={{ opacity: 1 }}><h3>Step 5: Rounds 2–16</h3><HintBox step={5} /><Step5Rounds2to16 L0Bits={state.L0} R0Bits={state.R0} roundKeys={state.roundKeys} onFinalValuesCalculated={() => {}} /><button onClick={() => { unlockNextStep(); setStep(6); }} style={{ marginTop: '1rem' }}>Next Step</button></motion.div>}
        ...
        {step === 8 && <motion.div key="step8" initial={{ opacity: 0 }} animate={{ opacity: 1 }}><h3>Step 8: Ciphertext Output</h3><HintBox step={8} /><Step8CiphertextOutput inputBits={state.inputBits} cipherBits={state.cipherBits} keyBits={state.keyBits} asciiOutput={state.asciiOutput} /></motion.div>}
      </AnimatePresence>
    </div>
  );
};
